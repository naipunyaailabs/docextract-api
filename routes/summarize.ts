...
export async function summarizeHandler(req: Request): Promise<Response> {
  ...
  if (!file || !(file instanceof File)) {
    return new Response("<div class='p-4 bg-red-50 text-red-700 rounded-lg text-center max-w-2xl mx-auto mt-8'>Error: No document provided or invalid file</div>", { 
      status: 400,
  ...
  const summarizationPrompt = userPrompt.trim() 
